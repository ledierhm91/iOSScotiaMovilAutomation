...
public class UI_Comun {
	...
	return (MobileElement) Config.driver.findElement(By.xpath(xpath));
	}
	
	public MobileElement findElementByID(String id) {
		return (MobileElement) Config.driver.findElementByAccessibilityId(id);
		}
	...
	// Metodo refectorizado
		public void swipeHLR() throws Exception {
			...
			new TouchAction(Config.driver).press(point(startPoint, anchor)).waitAction(waitOptions(ofMillis(1000)))
	...
	public void swipeV() throws Exception {
		...
		new TouchAction(Config.driver).press(point(anchor, startPoint)).waitAction(waitOptions(ofMillis(1000)))
	...
	public void swipeVsuc() throws Exception {
		...
		new TouchAction(Config.driver).press(point(anchor, startPoint)).waitAction(waitOptions(ofMillis(500)))
				.moveTo(point(anchor, endPoint)).release().perform();
	}
	...
	public void swipeVsuc3() throws Exception {
		...
		new TouchAction(Config.driver).press(point(anchor, startPoint)).waitAction(waitOptions(ofMillis(1500)))
				.moveTo(point(anchor, endPoint)).release().perform();
	}
	...
	public void tapPF() throws Exception {
		...
		new TouchAction(Config.driver).press(point(anchor, alto)).waitAction(waitOptions(ofMillis(1000))).release()
				.perform();
	}
	...
	public void home() throws Exception {
		...
		// c.swipeHRL();
		// c.swipeHLR();
		MobileElement home = (MobileElement) Config.driver.findElement(By.xpath("//XCUIElementTypeApplication[@name=\""
				+ Frmwrk.NombreApp
	...
	public void swipeHLRcorto() throws Exception {
		...
		// System.out.println(size.height+ "_"+ size.width + "_"+ anchor + "_"+
	...
	public void tapByCoordinates(int x, int y) {
		new TouchAction(Config.driver).tap(point(x, y)).waitAction(waitOptions(Duration.ofMillis(250))).perform();
	}
	...
	public void pinch() throws Exception {
		...
		/*
		 * new TouchAction(Config.driver) .press(point(alto, ancho))
		 * .waitAction(waitOptions(ofMillis(1000))) .moveTo(point(alto, ancho))
		 * .release().perform();
		 */
		// Zoom
		...
		Thread.sleep(8000);
		/*
		 * //Pinch TouchAction finger3= new TouchAction(Config.driver);
	...
	public void zoomIN() throws Exception {
		MobileElement btn = (MobileElement) Config.driver
				.findElement(By.xpath("//android.widget.ImageView[@content-desc=\"Acercar\"]"));
		btn.click();
	...
	public void zoomOUT() throws Exception {
		MobileElement btn = (MobileElement) Config.driver
				.findElement(By.xpath("//android.widget.ImageView[@content-desc=\"Alejar\"]"));
		btn.click();
	...
	public void clickOK() throws Exception {
		...
		MobileElement btnok = (MobileElement) Config.driver
				.findElement(By.xpath("//XCUIElementTypeButton[@name=\"OK\"]"));
		btnok.click();
	}
